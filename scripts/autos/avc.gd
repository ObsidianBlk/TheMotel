...
func _UpdateAudioFromSettings() -> void:
	...
	volume = Settings.get_value(SETTINGS_SECTION_AUDIO, AUDIO_BUS_MASTER, DEFAULT_BUS_VOLUME_MUSIC)
	_SetBusVolume(AUDIO_BUS_MUSIC, volume)
	
	volume = Settings.get_value(SETTINGS_SECTION_AUDIO, AUDIO_BUS_MASTER, DEFAULT_BUS_VOLUME_SFX)
...
func set_bus_volume(bus : StringName, volume : float, large_val : bool = false) -> void:
	...
	bus_volume_changed.emit(bus, volume * VOLUME_MULTIPLIER)
